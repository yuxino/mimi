...
const S={settings,session,calls:[],timeline:[],index:0,startedAt:null,controlMode:'island',pauseAt:null,pausedMillis:0};window.demoState=S;
...
function snapshot(){emit('session-state',session)}
function updateSettings(draft){Object.assign(settings,draft);S.timeline.push({kind:'settings',at:elapsed(),values:draft});emit('settings-changed',settings)}
...
window.invoke=async(kind,c,a={})=>{
 ...
 if(c==='overlay_show'){showWindow('overlay');return null}
