...
- `scripts/package-app.sh`：在 `tauri build` 前把所选身份交给 Tauri，使 `.app` 与随后创建的 `.dmg` 使用同一个本机身份；构建后通过 `codesign --verify --deep --strict`。GitHub 发布包不使用这一本机身份，而是每次构建使用临时签名；它们不保证跨版本的 TCC 或钥匙串身份连续性。
