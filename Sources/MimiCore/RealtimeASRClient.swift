...
public actor RealtimeASRClient {
    ...
    private let endpoint: RealtimeASREndpoint
    ...
    public init(
        ...
        self.endpoint = try RealtimeASREndpoint(workspaceID: workspaceID)
    ...
    public func connect(onEvent: @escaping EventHandler) async throws {
        ...
        let configuration = try RealtimeASRRequestEncoder.sessionUpdate(
            sourceLanguage: sourceLanguage
        )
    ...
    public func sendAudio(_ pcmData: Data) async throws {
        ...
        let message = try RealtimeASRRequestEncoder.audioAppend(pcmData)
    ...
    public func ping(timeout: Duration = .seconds(4)) async throws {
        ...
        let completion = ASRPingCompletion()
    ...
    public func finish(timeout: Duration = .seconds(2)) async {
        ...
        do {
            let message = try RealtimeASRRequestEncoder.finish()
            try await send(message, on: socket)
        ...
        while !Task.isCancelled {
            do {
                ...
                case let .string(text):
                    event = try RealtimeASRServerEventDecoder.decode(text)
                case let .data(data):
                    event = try RealtimeASRServerEventDecoder.decode(data)
                @unknown default:
...
private final class ASRPingCompletion: @unchecked Sendable {
