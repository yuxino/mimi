...
当前 macOS 发布版使用临时签名，不同构建的签名身份会变化。更新版本，或在本地构建与 GitHub 发布版之间切换，都可能要求重新授权。删除本地签名证书不会改变已安装应用的身份，也不能解决这个问题。正式版请放在 `/Applications/mimi.app`，开发测试使用独立的 `mimi-dev.app`。
...
**钥匙串要求密码或访问已保存的 API Key：**先确认请求来自你准备使用的 Mimi。如果弹窗提供「始终允许」，它可以为当前构建保留访问许可，但无法保证下次临时签名更新后不再询问。不要删除已保存的凭证、证书或登录钥匙串，不要允许所有应用访问密钥，也不要重置整个系统的权限。`codesign` 请求访问开发签名私钥是另一种构建时弹窗。

以上处理可用于恢复当前安装的访问权限，不能保证后续更新永不弹窗。解决发布版身份变化需要稳定的 Developer ID 签名，目前尚未上线。
