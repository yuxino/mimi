...
[![mimi — デモ](docs/demos/preview.gif)](docs/demos/demo.mp4)

[動画（MP4）](docs/demos/demo.mp4) · [デモについて](docs/demos/README.md)

字幕の言語・サイズ・配置・没入モードの設定。 実際のフロントエンドをサンプルデータで操作した録画です。内蔵のブラウザープレビューを使用し、実際の音声認識や翻訳結果は含みません。
<!-- /project-demo-v1 -->
...
**v1.3.8 は、アプリ内アップデートに対応する最初の公開導入版です。** 直前の公開版 v1.3.6 から更新する場合は、GitHub Releases から一度だけ手動でダウンロードしてインストールする必要があります。それ以降は「設定 → ソフトウェアアップデート」から更新できます。Windows では更新のインストール時に Mimi が終了するため、完了後に手動で開いてください。
