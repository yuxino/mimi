...
- **Windows プレビュー**：GitHub Releases では、Authenticode 未署名の x64 MSI と NSIS インストーラーを提供するため、Windows Defender SmartScreen が警告を表示する場合があります。実機でのインストール、信頼警告、認証情報ストレージ、システム音声取得、字幕ウィンドウ、字幕フロー全体はまだ検証していません。
