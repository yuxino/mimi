...
const SETTINGS_ZH = {
  ...
  retainSessionHistory: "保留本次字幕以便导出",
  retainSessionHistoryHelp: "保留已确认的原文与译文，停止后可导出 TXT。时间戳为字幕确认时距会话开始的时间，不是媒体播放时间。",
  recordSessionAudio: "录制本次系统音频",
  recordSessionAudioHelp: "停止后可导出单声道 WAV。暂停和重连期间的空白会被省略，录音时间不与字幕时间戳对齐。",
...
const SETTINGS_EN = {
  ...
  retainSessionHistory: "Keep session subtitles for export",
  retainSessionHistoryHelp: "Keep confirmed source and translation pairs for TXT export after stopping. Timestamps mark confirmation time since session start, not media playback time.",
  recordSessionAudio: "Record session system audio",
  recordSessionAudioHelp: "Export a mono WAV after stopping. Pauses and reconnect gaps are omitted, so audio timing does not align with transcript timestamps.",
...
const SETTINGS_JA = {
  ...
  retainSessionHistory: "書き出し用に今回の字幕を保持",
  retainSessionHistoryHelp: "確定した原文と訳文を保持し、停止後に TXT で書き出せます。時刻はセッション開始から字幕確定までの経過時間で、メディアの再生位置ではありません。",
  recordSessionAudio: "今回のシステム音声を録音",
  recordSessionAudioHelp: "停止後にモノラル WAV で書き出せます。一時停止や再接続中の空白は省かれるため、音声の時間は字幕のタイムスタンプと一致しません。",
