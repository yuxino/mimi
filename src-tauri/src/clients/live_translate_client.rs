...
impl LiveTranslateClient {
    ...
    pub async fn connect(&self) -> Result<(), LiveTranslateClientError> {
        ...
        let mut request = self.endpoint.url.clone().into_client_request().map_err(|_| {
            LiveTranslateClientError::NotConnected
        })?;
        ...
        *self.inner.sink.lock().await = Some(sink);
        self.inner.received_session_finished.store(false, Ordering::SeqCst);
            ...
            loop {
                ...
                match message {
                    Ok(Message::Text(text)) => {
                        match LiveTranslateServerEvent::decode(&text) {
                            Ok(event) => {
                                if event == LiveTranslateServerEvent::SessionFinished {
                                    inner.received_session_finished.store(true, Ordering::SeqCst);
                                }
